...
6% to LP - done
2% reflect - done
2% marketing wallet - done

max holding 1% - done
max transaction 0.5% - done


marketing: 0x11f370DF4503c43058b166c14b01ee5808ec0510
owner: 0x313234BC9fb2920dbD3aB9BcbA71Dbc1e102818C
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x313234BC9fb2920dbD3aB9BcbA71Dbc1e102818C;
...
contract TheVigilantesToken is Context, IERC20, Ownable {
    ...
    // only team can buy & sell, for testing
    mapping (address => bool) public _isTeam;

    // default set, only team can buy at start
    bool public onlyTeam = true;
    ...
    address payable private _promotionsWalletAddress = payable(0x11f370DF4503c43058b166c14b01ee5808ec0510);
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10 * 10**9 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "VIGILANTES V4"; 
    string private _symbol = "VIGILANTES";
    uint8 private _decimals = 9;
    
    //refection fee - The redistribution, gives more tokens to all holders! = happy holders :)
    uint256 public _reflectionFee = 2;
    uint256 private _previousReflectionFee = _reflectionFee;
    
    //token wallet fee for burns and giveaways
    //this is set to 0, so it will not take any tokens, but if you want to use it you can change it later
    //it is useful to let it build up with tokens that you can use for a manual burn, this will help to 
    //recover the $ value if the price drops. It's also good for doing token giveaways
    ...
    //this is silly maths because solidity maths is kinda annoying at times 
    //whatever you put into these two values doesn't really matter, it is the total of these two values 
    //that is important. The numbers here are only to show the value for people reading the contract on 
    //BSCscan. Here we have 6 and 2. In total that is 8. So later we need to split these numbers to get
    //a 6 and 2.... We do this by dividing the total 8 by 4 (which is the _liqToPromoFraction number auto
    //few lines down) We process the marketing wallet first, so we divide the total (_liquidityAndPromoFee 8)
    //by the (_liqToPromoFraction 4) This gives us the 2 that we need to process the marketing wallet, then 
    //we use the remainder (6) for the liquidity
    //Doing anything funky with math seems to cause a lot of problems in solidity, so this is a very basic 
    //way to solve this problem. But for any coders reading this.... I'm embarrased! :)
    ...
    //SET TO 4: 6+2=8 8/4=2 This will take 2% for the marketing wallet and (8-2=6) 6% for liquidity
    uint256 private _liqToPromoFraction = 4;
    
    //max wallet holding 10000000 * 10**6 * 10**9 is 1% of 1000000000 * 10**6 * 10**9 (total supply)
    //We have two ways to change this the function setMaxWalletPercent will allow you to set the max wallet
    //holding limit as a percent of the total supply, or you can use the setMaxWalletTokens function to 
    //set the limit to a specific number of tokens. But... remeber all the 0's! There are 9 0's after the decimal
    //and you need to add them all so in tokens 1% is 10000000000000000000000
    //I wish all computers automatically made every 3rd 0 bold! Counting all those 0's is impossible! 
    ...
    //this is the maximum transaction amount (in tokens) at launch this is set to 0.5% of total supply
    //You can change this with a % value using the function setMaxTxPercent - BUT I think this will only accept whole numbers!
    //So you can also change it using the setMaxTxTokens function, to set the amount in tokens 
    //0.5% in tokens is 5000000000000000000000 <---because you just ignore the decimal, for some weird reason!
    ...
    //as the price goes up, you might want to lower this amount, so it adds liquidity more often
    ...
    constructor () {
        ...
        _isExcludedFromFee[address(0x11f370DF4503c43058b166c14b01ee5808ec0510)] = true;
        _isExcluded[address(0x11f370DF4503c43058b166c14b01ee5808ec0510)] = true;
    ...
    //set Only Team  Members 
    function setonlyTeam(bool _enabled) public onlyOwner {
        onlyTeam = _enabled;
    }
    ...
    //as the price goes up, people buy less tokens for teh same money, so you might want to lower
    //this amount over time to keep a steady flow into the liquidity 
    ...
    //Remove from Team 
    function removeFromTeam(address account) external onlyOwner {
        _isTeam[account] = false;
    }
    ...
    function batchRemoveFromBlackList(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _isBlacklisted[addresses[i]] = false;
      }    
    }

    //adding people to the Team approved list - these people are the only ones that will be able to buy at launch! 
    function addToTeam(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _isTeam[addresses[i]] = true;
      }
    }
    ...
    function _takeDev(uint256 tDev) private {
        ...
        emit Transfer(_msgSender(), _promotionsWalletAddress, tDev);
    ...
    function _transfer(
        ...
        //limits the amount of tokens that each person can hold in total - launch limit is 1% of total supply!
        if (to != owner() && 
            to != address(this)  && 
        ...
        //if onlyTeam is set to true, then only people that have been approved as part of the team can buy 
        if (onlyTeam && from != owner() ){
        require(_isTeam[to], "Only Team Can Buy/Sell! Trading not open yet");}
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
       ...
       //this is where we use that weird fraction! so it splits the total and does the marketing wallet first
       //this gives us the smaller amount, it sends that portion to the marketing wallet then uses the rest for
       //the liquidity
